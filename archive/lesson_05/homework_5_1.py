...
filtered_cars = []
for car_name, (color, year, engine, model, price) in car_data.items():
  if year >= min_year and engine >= min_engine_volume and price <= max_price:
    filtered_cars.append((car_name, color, year, engine, model, price))
sorted_cars = sorted(filtered_cars, key=lambda x: x[5])
print("Знайдені авто:")
for car in sorted_cars[:5]:
  name, color, year, engine, model, price = car
  print(f'{name}: {color} {year} {engine} {model} {price}')